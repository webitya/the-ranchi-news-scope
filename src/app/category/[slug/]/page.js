...
export default function CategoryPage() {
    ...
    return (
        <div style={{ background: 'var(--color-bg-primary)', paddingTop: '160px' }}>
            <header className="container" style={{ marginBottom: 'var(--spacing-2xl)' }}>
                <div style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 'var(--spacing-md)',
                ...
                <p style={{ fontSize: '1.25rem', color: 'var(--color-text-secondary)' }}>
                    Stay updated with the latest {categoryName.toLowerCase()} news and trends from Ranchi and beyond.
                </p>
            </header>

            <div className="container" style={{ minHeight: '60vh' }}>
                {articles.length > 0 ? (
                    <NewsGrid articles={articles} title={`All in ${categoryName}`} />
                ) : (
